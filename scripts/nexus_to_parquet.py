#!/usr/bin/env python3
"""
NeXus HDF5 to Parquet Ingestion Script

This script reads NeXus format HDF5 files (commonly used in neutron scattering
experiments) and converts all data into Parquet files organized by data category:
...
Output Parquet files:
  ...
  - events.parquet: Neutron detector event data
  - sample.parquet: Sample information
  - instrument.parquet: Instrument configuration
  - users.parquet: User/experimenter information

Usage:
  python nexus_to_parquet.py /path/to/file.nxs.h5
  python nexus_to_parquet.py /path/to/file.nxs.h5 --output-dir /path/to/output
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert NeXus HDF5 files to Parquet format',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s ~/data/REF_L_218389.nxs.h5
  %(prog)s ~/data/REF_L_218389.nxs.h5 --output-dir ./parquet_output
  %(prog)s ~/data/REF_L_218389.nxs.h5 --max-events 100000
  %(prog)s ~/data/REF_L_218389.nxs.h5 --no-events
        """
    )
    
    parser.add_argument(
        'input_file',
        help='Path to the NeXus HDF5 file'
    )
    parser.add_argument(
        '--output-dir', '-o',
        default=None,
        help='Output directory for Parquet files (default: same as input file)'
    )
    parser.add_argument(
        '--max-events', '-m',
        type=int,
    ...
    parser.add_argument(
        '--no-events',
        action='store_true',
        help='Skip event data extraction (useful for large files)'
    )
    
    args = parser.parse_args()
    
    # Expand user path
    input_file = os.path.expandvars(os.path.expanduser(args.input_file))
    ...
    if args.output_dir:
        ...
        if not output_dir:
            output_dir = '.'
        output_dir = os.path.join(output_dir, 'parquet_output')
    ...
    for data_type, path in output_files.items():
        ...
        print(f"  {data_type}: {os.path.basename(path)} ({size_str})")
